...
Config::Config(int argc, LPCWSTR argv[], HRESULT &hr)
: m_pMMDevice(NULL)
, m_hFile(NULL)
, m_bInt16(false)
, m_pwfx(NULL)
...
HRESULT Config::get_default_device(IMMDevice **ppMMDevice) {
	HRESULT hr = S_OK;
...
HRESULT Config::get_specific_device(LPCWSTR szLongName, IMMDevice **ppMMDevice) {
	HRESULT hr = S_OK;
...
HRESULT Config::open_file(LPCWSTR szFileName, HMMIO *phFile) {
	MMIOINFO mi = { 0 };
